...
sudo aptitude -y install xorg fluxbox slim rxvt-unicode xdg-utils vim-gtk alsa-utils moc moc-ffmpeg-plugin network-manager libgconf2-4 libxss1 libcurl3 keychain flashplugin-nonfree google-chrome-stable >> ~/install.log && sudo aptitude -y -t squeeze-backports install iceweasel icedove >> ~/install.log &
