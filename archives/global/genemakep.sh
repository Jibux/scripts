...
main=${CODE[0]}".o: "${CODE[0]}".cpp" #Dépendances (.h et .cpp) du programme.o
ajoutmain=$main" "

for (( i=0; i <= n; i++ ))
do
	line=${CODE[$i]}".o: "${CODE[$i]}".cpp " #Dépendances (.cpp) des dépendances.o
...
sed -i "s/$main/$ajoutmain/g" $path #On remplace par la ligne avec ses dépendances .h et .cpp

#Ecriture des fichiers .cpp et .h

includes="#include <iostream>\n#include <sstream>"

if [ ! -e "$path1${CODE[0]}.cpp" ]
then
	prog="true"
else
	prog="false"
...
for (( i=0; i <= n; i++ ))
do
	if [ ! -e "$path1${CODE[$i]}.cpp" ]
	then
		echo -e "$includes" > $path1${CODE[$i]}".cpp"

		if [ $i -gt 0 ]
		then
			echo -e "#include \"${CODE[$i]}.h\"" >> $path1${CODE[$i]}".cpp"
			if [ $prog = "true" ]
			then
				echo -e "#include \"${CODE[$i]}.h\"" >> $path1${CODE[0]}".cpp"
			fi
